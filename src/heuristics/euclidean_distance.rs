...
pub fn euclidean_distance(current: &Node, goal: &Node) -> i32 {
    ...
    for i in 0..puzzle_size {
        for j in 0..puzzle_size {
            ...
            if tile != 0 {
                // Don't calculate distance for the empty space (assuming 0 is the empty space)
                // Find the goal position of this tile
                let (goal_x, goal_y) = find_position(&goal.puzzle_configuration, tile);
                // Calculate the Euclidean distance for this tile
                let dx = (i as f64 - goal_x as f64).powi(2);
                let dy = (j as f64 - goal_y as f64).powi(2);
                distance += (dx + dy).sqrt();
            }
        }
    }
...
fn find_position(goal: &Vec<Vec<i32>>, tile: i32) -> (usize, usize) {
    ...
    (0, 0) // Fallback, shouldn't occur if tile is always in the puzzle
}